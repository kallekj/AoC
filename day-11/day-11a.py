...
class Monkey:
    ...
    def inspect_next_item(self):
        ...
        match self.worry_level_operator:
            case '+':
                worry_level = item + modifier_value_parser(self.worry_level_modifier_value, item)
                printer(f'Worry level increases by {modifier_value_parser(self.worry_level_modifier_value, item)} to {worry_level}.')
            # case '-':
            #     worry_level = item - modifier_value_parser(self.worry_level_modifier_value, item)
            case '*':
                worry_level = item * modifier_value_parser(self.worry_level_modifier_value, item)
                printer(f'Worry level is multiplied by {modifier_value_parser(self.worry_level_modifier_value, item)} to {worry_level}.')
            # case '/':
            #     worry_level = item / modifier_value_parser(self.worry_level_modifier_value, item)
            case _:
                worry_level += 0
        ...
        printer(f'Monkey gets bored with item. Worry level is divided by 3 to {worry_level}')
        return worry_level

    def throw_next_item(self, item:int, monkeys:list):
        if item % self.test_divisible == 0:
            printer(f'Current worry level is divisible by {self.test_divisible}.')
            monkey:Monkey = monkeys[self.throw_to_monkey_if_true]
            printer(f'Item with worry level {item} is thrown to monkey {monkey.index}')
            monkey.add_item(item)
        else:
            printer(f'Current worry level is not divisible by {self.test_divisible}.')
...
for i, instruction in enumerate(instructions.values()):
    if type(instruction['Starting items']) == int:
        ...
        starting_items = [int(item) for item in instruction['Starting items'].split(', ')]
    ...
    monkeys.append(Monkey(i, starting_items, divisible_by, worry_level_modifier_value, worry_level_operator, if_true, if_false))
...
for i in range(1, 21):
    ...
    printer(f'Round {i} \n')
    for monkey in monkeys:
        printer(f'Monkey {monkey.index}: {monkey.items}')
    printer('\n\n')


for monkey in monkeys:
    print(f'Monkey {monkey.index} inspected items {monkey.inspected_items} times.')
