...
function checkIncorrectUpdate(update: number[], rules: string[]){
    let counter = 0;
...
async function main() {
    ...
    const incorrectUpdates = updates.map(update => checkIncorrectUpdate(update, rules)).filter(update => update !== null);
    
    const sortedUpdates =incorrectUpdates.map(incorrectUpdate => incorrectUpdate.sort((a,b) => checkPageUpdate(incorrectUpdate, a, b, rules) ? 1 : -1));
    const middleNumbers = sortedUpdates.map(update => update ? update[(update?.length-1)/2] : 0);
